...
(deftest initiate-current-item!
  (mc/initiate-current-item! (g/build-point 2 3))
  (is (= (:input (:item (d/current))) (g/build-rect (g/build-point 2 3) (g/build-point 2 3)))))

(deftest update-current-item!
  (mc/initiate-current-item! (g/build-point 2 3))
  (mc/update-current-item! (g/build-point 4 5))
  (is (= (:input (:item (d/current))) (g/build-rect (g/build-point 2 3) (g/build-point 4 5)))))

(deftest finish-current-item!
  (mc/initiate-current-item! (g/build-point 2 3))
  (mc/update-current-item! (g/build-point 4 5))
...
(deftest locking
  (m/set-tool! :rect)
  (let [id1 (create-item! (g/build-rect 3 3 13 13))]
    (m/set-tool! :line)
