...
(deftest handle-input-event-action-down
  (di/handle-input-event {:type :down, :data {:action :resize-ne, :point (p/coords->position (g/build-point 2 4))}})
  (is (= (d/action) :resize-ne)))

(deftest handle-input-event-action-up
  (di/handle-input-event {:type :up, :data {:action :resize-ne, :point (p/coords->position (g/build-point 2 4))}})
  (is (= (d/action) nil)))

(deftest handle-input-event-draw-down
  (di/handle-input-event {:type :down, :data {:action :draw, :point (p/coords->position (g/build-point 2 4))}})
  (is (= (:input (:item (d/current))) (g/build-rect 2 4 2 4))))

(deftest handle-input-event-draw-up
  (di/handle-input-event {:type :down, :data {:action :draw, :point (p/coords->position (g/build-point 2 4))}})
  (di/handle-input-event {:type :up, :data {:action :draw, :point (p/coords->position (g/build-point 2 4))}})
...
(deftest handle-input-event-select-down-edit-text
  (let [id (create-layer! (g/build-rect 2 2 4 4))]
    ...
    (di/handle-input-event {:type :down, :data {:action :draw, :point (p/coords->position (g/build-point 2 2))}})
    (di/handle-input-event {:type :down, :data {:action :draw, :point (p/coords->position (g/build-point 2 2))}})
    (di/handle-input-event {:type :up,   :data {:action :draw, :point (p/coords->position (g/build-point 2 2))}})
...
(deftest handle-input-event-select-up
  ...
  (di/handle-input-event {:type :up, :data {:action :draw, :point (p/coords->position (g/build-point 5 5))}})
  (is (= (d/selected-ids) [0])))
...
(deftest handle-mousemove-resize
  ...
  (di/handle-input-event {:type :move, :data {:point (p/coords->position (g/build-point 6 6))}})
